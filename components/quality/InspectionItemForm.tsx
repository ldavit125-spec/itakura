  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <h5 className="text-xs font-bold text-gray-800">■ 개별 검사 항목 측정 및 적합성 결과</h5>
        {!readOnly && (
          <span className="text-[11px] text-gray-500 font-medium">
            * 부적합(FAIL) 항목이 1개 이상이면 합격 판정이 제한됩니다.
          </span>
        )}
      </div>
      ...
      <div className="overflow-x-auto border border-gray-200 rounded-lg bg-white">
        <table className="w-full text-xs text-left text-gray-700">
          <thead className="bg-gray-50 text-gray-500 uppercase border-b border-gray-200">
            <tr>
              <th className="px-3 py-2 font-semibold">검사 항목명</th>
              <th className="px-3 py-2 font-semibold">기준값 (Standard)</th>
              <th className="px-3 py-2 font-semibold">측정값 (Measured)</th>
              <th className="px-3 py-2 font-semibold text-center">결과 판정</th>
              <th className="px-3 py-2 font-semibold">비고 / 비고사항</th>
            </tr>
          </thead>
              ...
              <tr key={idx} className="hover:bg-gray-50">
                ...
                <td className="px-3 py-2 text-center whitespace-nowrap">
                  {readOnly ? (
                    ...
                    >
                      {item.result === "PASS" ? "적합 (PASS)" : item.result === "FAIL" ? "부적합 (FAIL)" : "해당 없음"}
                    </span>
                  ) : (
                    <select
                      value={item.result}
                    ...
                    >
                      ...
                      <option value="NOT_APPLICABLE">해당없음 (N/A)</option>
                    </select>
                  )}
                </td>
                <td className="px-3 py-2">
