    ...
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      ...
      {tab === "register" && (
        <form onSubmit={submit} className="p-5">
          <div className="mb-5 flex items-center justify-between"><div><h3 className="font-bold text-gray-900">{t("shipment.register.title")}</h3><p className="mt-1 text-xs text-gray-500">{t("shipment.register.description")}</p></div><span className="rounded-lg bg-gray-100 px-3 py-2 text-xs font-mono text-gray-600">{t("shipment.register.autoNumber")}</span></div>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <Field label={`${t("shipment.field.lotNumber")} *`}><select value={lotNumber} onChange={(e) => setLotNumber(e.target.value)} required className={inputClass}><option value="">{t("shipment.placeholder.selectLot")}</option>{lotAvailability.map((lot) => <option key={lot.lotNumber} value={lot.lotNumber}>{lot.lotNumber} · {displayName(locale, lot.productName, lot.productNameJa)} · {t("shipment.available")} {lot.availableQuantity.toLocaleString()}{t("unit.item")}</option>)}</select></Field>
            ...
            <Field label={`${t("shipment.field.plannedDate")} *`}><input type="date" value={plannedDate} onChange={(e) => setPlannedDate(e.target.value)} required className={inputClass} /></Field>
...
function ShipmentList({ items, history = false, productJaByCode, customerJaByName, onOpen, actions }: { items: Shipment[]; history?: boolean; productJaByCode: Map<string, string | null>; customerJaByName: Map<string, string | null>; onOpen: (item: Shipment) => void; actions?: (item: Shipment) => React.ReactNode }) {
  const { t, locale } = useLanguage();
